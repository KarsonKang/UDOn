    ...
    if key < 11:
        ...
        if buff[1] == 0x51:
            ...
            if check_sum(data_buff[0:10], data_buff[10]):
                ...
                acceleration = [hex_to_short(data_buff[2:10])[i] for i in range(0, 3)]
                angle_flag = True
            else:
            ...
            if check_sum(data_buff[0:10], data_buff[10]):
                ...
                angularVelocity = [hex_to_short(data_buff[2:10])[i] for i in range(0, 3)]
                angle_flag = True
            else:
            ...
            if check_sum(data_buff[0:10], data_buff[10]):
                magnetometer = [hex_to_short(data_buff[2:10])[i] / 32768.0 for i in range(0, 3)]
                # angle_flag = True
            else:
...
class IMUDriverNode(Node):
    def __init__(self, port_name):
        ...
        # self.timer = self.create_timer(0.02, self.driver_loop(port_name))
        ...
        while True:
            ...
            try:
                ...
                if buff_count > 0:
                    ...
                    # self.get_logger().info(f"Processed {buff_count} bytes data, time cost: {elapsed_time:.6f}")

    def imu_data(self):
        accel_x, accel_y, accel_z = acceleration[0], acceleration[1], acceleration[2]  # struct.unpack('hhh', accel_raw)
        accel_scale = 16 / 32768.0 * 9.8
        ...
        # mag_x, mag_y, mag_z = magnetometer[0] * 7000 / 32768.0, magnetometer[1] * 7000 / 32768.0, magnetometer[2] * 7000 / 32768.0
        mag_x, mag_y, mag_z = magnetometer[0] / 1.0, magnetometer[1] / 1.0, magnetometer[2] / 1.0
